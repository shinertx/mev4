# /src/core/agent.py
# FINAL VERSION: An async orchestrator for a single, stateful strategy.
...
class Agent:
    ...
    async def run_loop(self):
        """The main async execution loop for the agent."""
        log.info("STRATEGIC_AGENT_STARTING_LOOP", strategy=self.strategy_name)

        while not is_kill_switch_active():
            try:
                # 1. Check for and apply any approved mutations first
                await self.strategy.mutate(self.adapters)
                ...
                if now - self.last_mutation_request_time > self.mutation_request_interval:
                    ...
                    # This call requires the strategy to have a 'get_performance_data' method
                ...
                log.error("STRATEGIC_AGENT_LOOP_ERROR", strategy=self.strategy_name, error=str(e), exc_info=True)
                await asyncio.sleep(self.run_interval) # Wait before retrying
        
        log.critical("STRATEGIC_AGENT_HALTED_BY_KILL_SWITCH", strategy=self.strategy_name)
